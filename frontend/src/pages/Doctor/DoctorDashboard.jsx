  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 w-full text-left max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-zinc-200/80 dark:border-zinc-800 pb-4 gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-zinc-950 dark:text-zinc-50 font-sans">Provider Administration</h1>
          <p className="text-zinc-500 dark:text-zinc-400 text-xs mt-0.5">Clinical metrics, consultation schedule, and smart prescription generator.</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              {/* Stats widgets */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 p-5 rounded-2xl flex items-center justify-between shadow-sm">
                  <div className="space-y-1">
                    <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Gross Income</span>
                    <h3 className="text-2xl font-extrabold text-zinc-900 dark:text-zinc-50">₹{dashData.earnings}</h3>
                  </div>
                ...
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 p-5 rounded-2xl flex items-center justify-between shadow-sm">
                  <div className="space-y-1">
                    <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Consultations</span>
                    <h3 className="text-2xl font-extrabold text-zinc-900 dark:text-zinc-50">{dashData.appointments}</h3>
                  </div>
                ...
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 p-5 rounded-2xl flex items-center justify-between shadow-sm sm:col-span-2 lg:col-span-1">
                  <div className="space-y-1">
                    <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Unique Patients</span>
                    <h3 className="text-2xl font-extrabold text-zinc-900 dark:text-zinc-50">{dashData.patients}</h3>
                  </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 p-5 rounded-3xl shadow-sm space-y-4">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 flex items-center gap-1.5"><TrendingUp className="w-4 h-4 text-primary" /> Revenue Growth Log</h3>
                  {chartData ? (
                    <div className="h-[220px]">
                      <Line data={chartData} options={chartOptions} />
                    </div>
                  ) : (
                    <div className="h-[220px] flex items-center justify-center text-zinc-400 text-xs">No records available.</div>
                  )}
                </div>

                <div className="bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 p-5 rounded-3xl shadow-sm space-y-4">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 flex items-center gap-1.5"><Clock className="w-4 h-4 text-amber-500" /> Today's Queue</h3>
                      ...
                      dashData.latestAppointments.slice(0, 4).map((appt) => (
                        <div key={appt._id} className="py-2.5 flex items-center justify-between gap-3 text-xs">
                          <div className="flex items-center gap-2.5 truncate">
                            <img className="w-8 h-8 rounded-full object-cover border border-zinc-200 dark:border-zinc-700" src={appt.userData?.image || '/fallback-user.png'} alt="user" />
                            <span className="font-bold text-zinc-900 dark:text-zinc-100 truncate">{appt.userData?.name}</span>
                          </div>
                          <span className="text-[11px] font-semibold text-primary px-2 py-0.5 rounded-md bg-primary/10">{appt.slotTime}</span>
                        </div>
                      ))
                    ) : (
                      <p className="text-xs text-zinc-400 py-6 text-center">No queued patients today.</p>
                    )}
                  </div>
                </div>
              </div>
          ...
          {/* TAB 2: ROSTER LIST */}
          {activeTab === 'roster' && (
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 rounded-3xl p-4 sm:p-6 shadow-sm space-y-5">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3">
                <div className="relative flex-1 max-w-md">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2 bg-zinc-50/70 dark:bg-zinc-950/70 border border-zinc-200 dark:border-zinc-800 rounded-xl text-xs font-medium text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-primary/20"
                  />
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select 
                    ...
                    className="border border-zinc-200 dark:border-zinc-800 p-2 bg-zinc-50/70 dark:bg-zinc-950/70 rounded-xl text-xs font-bold text-zinc-700 dark:text-zinc-300 focus:outline-none"
                  >
              ...
              {/* DESKTOP VIEW: Roster Table */}
              <div className="hidden md:block overflow-x-auto rounded-2xl border border-zinc-200/80 dark:border-zinc-800">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-zinc-50/70 dark:bg-zinc-950/50 border-b border-zinc-200/80 dark:border-zinc-800 text-[11px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                  ...
                  <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/60">
                    {filteredAppointments.map((appt) => (
                      <tr key={appt._id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/40 transition-colors">
                        <td className="p-3.5 flex items-center gap-2.5">
                          <img className="w-8 h-8 rounded-full object-cover border border-zinc-200 dark:border-zinc-700" src={appt.userData?.image || '/fallback-user.png'} alt="" />
                          <span className="font-bold text-zinc-900 dark:text-zinc-100">{appt.userData?.name}</span>
                        </td>
                        <td className="p-3.5 font-medium">{slotDateFormat(appt.slotDate)} at <span className="font-bold text-zinc-900 dark:text-white">{appt.slotTime}</span></td>
                        <td className="p-3.5 font-semibold text-zinc-500 dark:text-zinc-400">{calculateAge(appt.userData?.dob)} yrs</td>
                        <td className="p-3.5">
                          <span className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-bold ${
                            appt.payment ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20'
                          }`}>
                        ...
                        <td className="p-3.5 text-right">
                          {!appt.cancelled && !appt.isCompleted && appt.videoLink && (
                            <a 
                              ...
                              className="inline-flex items-center gap-1 bg-emerald-500 hover:bg-emerald-600 text-white px-3 py-1.5 rounded-xl font-bold text-xs shadow-sm active:scale-95 transition-all"
                            >
              ...
              <div className="md:hidden space-y-3">
                {filteredAppointments.map((appt) => (
                  <div
                    key={appt._id}
                    className="p-4 rounded-2xl bg-zinc-50/70 dark:bg-zinc-950/60 border border-zinc-200/80 dark:border-zinc-800 space-y-3"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        <img className="w-9 h-9 rounded-full object-cover border border-zinc-200 dark:border-zinc-700" src={appt.userData?.image || '/fallback-user.png'} alt="" />
                        <div>
                          <p className="font-bold text-zinc-900 dark:text-white text-xs">{appt.userData?.name}</p>
                          <p className="text-[10px] text-zinc-400">Age: {calculateAge(appt.userData?.dob)} yrs</p>
                        </div>
                      </div>
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                        appt.cancelled ? 'bg-red-500/10 text-red-500' : appt.isCompleted ? 'bg-emerald-500/10 text-emerald-500' : 'bg-primary/10 text-primary'
                      }`}>
                        {appt.cancelled ? 'Cancelled' : appt.isCompleted ? 'Completed' : 'Upcoming'}
                      </span>
                    </div>

                    <div className="flex items-center justify-between text-xs pt-1 border-t border-zinc-200/60 dark:border-zinc-850">
                      <span className="text-zinc-500 dark:text-zinc-400 font-medium">Slot:</span>
                      <span className="font-bold text-zinc-900 dark:text-white">{slotDateFormat(appt.slotDate)} at {appt.slotTime}</span>
                    </div>
                    ...
                    {!appt.cancelled && !appt.isCompleted && appt.videoLink && (
                      <a 
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-full inline-flex items-center justify-center gap-1.5 bg-emerald-500 hover:bg-emerald-600 text-white py-2 rounded-xl font-bold text-xs shadow-sm transition-all"
                      >
                        <Video className="w-4 h-4" /> Telehealth Join
                      </a>
                    )}
                  </div>
                ))}
